  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {rfq.manufacturingProcess.name} · {rfq.material.name} · qty {rfq.quantity} · due{" "}
            {format(rfq.requiredDeliveryDate, "d MMM yyyy")}
          </p>
        </div>
      ...
      <section className="card space-y-2">
        <h2 className="section-title">Technical files</h2>
        {rfq.files.length === 0 ? (
          ...
          <ul className="space-y-1 text-sm">
            {rfq.files.map((file) => (
              <li key={file.id}>
                ...
                <span className="text-slate-400">({Math.ceil(file.sizeBytes / 1024)} KB)</span>
              </li>
            ))}
          </ul>
        )}
        ...
        {rfq.status === "OPEN" && (
          <>
            ...
            <ActionButton
              ...
              confirmMessage="Cancel this RFQ?"
            />
          </>
        )}
      </section>
      ...
      {rfq.status === "OPEN" && (
        <section>
          <h2 className="section-title mb-3">
            Matched suppliers — ranked by capability & availability
          </h2>
          {rfq.matches.filter((m) => m.compatible).length === 0 ? (
            <div className="card text-sm text-slate-500">
              No compatible machines found yet. This can mean no supplier currently has both the
              capability and the published capacity for this job.
            </div>
          ) : (
